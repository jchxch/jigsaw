    ...
    def save_jie3(self):  # 保存样本数据
        ...
        for k in self.dict:
            ...
            for j in range(self.N):
                data[i][j] = float(k[j])
            i += 1
        np.savez("jie3_1.npz", data=data, label=label)

    def save_q_tab(self):  # 保存Query表
        k = list(self.dict.keys())
        v = list(self.dict.values())
        np.savez("q_tab_1.npz", k=k, v=v)

    def save_q_tab_(self,path):  # 保存Query表
        k = list(self.dict.keys())
        v = list(self.dict.values())
        np.savez("q_tab_"+path+".npz", k=k, v=v)

    def save_txt(self):  # 保存为TXT文档
        file = open("jie3_1.txt", 'w')
        s = ""
        for k in self.dict:
            v = self.dict[k]
